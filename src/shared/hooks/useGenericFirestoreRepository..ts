...
export default function useGenericFirestoreRepository<T extends WithFieldValue<DocumentData>>(collectionName: string) {
  const queryClient = useQueryClient();
  const service = makeFirestoreRepository<T>(collectionName);
